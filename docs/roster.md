...
| Taillow | Catchable | `Wasteland_SouthRoad`, wild encounter (levels 12-17) | Early fast physical Flying attacker | Territory 3's connecting route (see the Thirty-ninth feature entry) - a genuinely new roster addition (not reused from an earlier map's table), picked for a fast physical role nothing else on the roster yet fills |
